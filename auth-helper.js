...
const BlackcessDB = {
    ...
    async sendNotificationEmail(type, to, data) {
        if (!to) return;
        try {
            const { error } = await window.supabase.functions.invoke("send-notification-email", {
                body: { type, to, data }
            });
            if (error) console.warn(`Notification email (${type}) failed to send:`, error.message);
        } catch (err) {
            console.warn(`Notification email (${type}) failed to send:`, err.message);
        }
    },
